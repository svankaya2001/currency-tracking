...
def create_currency():
    ...
    for index, currency in enumerate(currencies, 1):
        ...
        bid = currency.find("td", class_=f"pid-{index}-bid").text
        ask = currency.find("td", class_= f"pid-{index}-ask").text
        high = currency.find("td", class_= f"pid-{index}-high").text
        low = currency.find("td", class_= f"pid-{index}-low").text
        change = currency.find("td", class_= f"pid-{index}-pc").text
        change_p = currency.find("td", class_= f"pid-{index}-pcp").text
        time = currency.find("td", class_= f"pid-{index}-time").text
...
def update_currency():
    ...
    html = urlopen(req).read()
    bs = BeautifulSoup(html, 'html.parser')
    currencies = bs.find("tbody").find_all("tr")
